    ...
    for (var i = 0; i < fileDataString.length; i++) {
        if (fileDataString.charAt(i) == "=") {
            ...
            if (currentBar === null) {
                ...
                var result = parseDataString;
                var slice = 0;
                
                if (parseDataString.indexOf("*") > -1) {
                    currentBar.rehearsalLetter = parseDataString.charAt(parseDataString.indexOf("*") + 1);
                    slice += 2;
                }

                if (parseDataString.indexOf("T") > -1) {
                    currentBar.time = parseDataString.charAt(parseDataString.indexOf("T") + 1) + '/' + parseDataString.charAt(parseDataString.indexOf("T") + 2);
                    slice += 3;
                }
